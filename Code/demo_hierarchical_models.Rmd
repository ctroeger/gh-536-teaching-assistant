---
title: 'Hierarchical Model Demo: Sleep Study'
author: "Chris Troeger"
date: "January 3, 2022"
output: html_document
---

# This script is intended to demonstrate how to use and interpret a hierarchical linear model in R

Hierarchical models are models that incorporate grouping, clustering, or repeated measurements from a dataset into a statistical regression model. Other names for this model structure are:

* Mixed-effects models
* Random-effects (random intercepts; random slopes)
* Multi-level models

There are two main packages to run hierarchical (mixed-effects) models in R.  
...
##########################################
## Simple demonstration of hierarchical
## models
##########################################
library(data.table)
library(lme4)
...
So it seems that with each day of sleep deprivation, reaction time increases. Let's quantify by how much using a simple linear model. How do we interpret the results of the model? 
...
summary(my_lm) # Reaction time increases by 10 milliseconds per day
...
Our model isn't too bad for most Subjects, but there is variation in its fit. Let's see if we can improve it (and our uncertainty) by considering the data structure. 

## Now, we will run our hierarchical model

The key idea of a hierarchical model is that the data are grouped or structured in some way. Take these data for example, there are multiple observations for each Subject. To correctly account for this variation in the estimates of our effect sizes and standard errors, we should include this information in the model. Often, these models are written
...
In the lme4 package, we will use the function $lmer$ to run a hierarchical model. We specify random intercepts like this:
...
The other package for hierarchical models that is commonly used, $nlme$ has some nice features that may be useful later in the class. For now though, let's just see how to run a model with that package.
